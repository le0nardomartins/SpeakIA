...
- `preload.js` is the only bridge and exposes exactly 10 methods
...
| `listConversations()` | `speakai:list-conversations` | `loadAllTalkSummaries()` | Returns list of saved conversations with preview |
...
### `listConversations` response shape
...
[
  {
    "talkNumber": 1,
    "fileName": "talk_1.txt",
    "preview": "First 300 characters of the summary..."
  }
]
```
...
bootstrap()
  ...
  registerIpcHandlers()    — 10 ipcMain.handle() registrations
  app.whenReady()
...
Returns a `data:audio/mp3;base64,...` URL ready for the custom audio player element.
...
- `renderApiKeysState(settings)` — shows a green "Chave configurada" indicator under each input when a key exists; updates placeholder text; stores the current state in `_apiKeysExist`
- `saveApiKeys()` — if existing keys would be overwritten, shows a red confirmation modal (`#apiKeyOverwriteModal`) before proceeding
- `_doSaveApiKeys(openai, elevenlabs)` — the actual save logic, called after confirmation
...
Returns the wrapper `div` element. For `role === "assistant"`:
- Appends a globe-icon translate button (`<button class="msg-translate-btn">`)
- If `animate === true`: reveals text word-by-word via `typewriterReveal()` (28ms/word); translation div is appended after animation completes
- First translate click: calls `window.speakAI.translateText()`, appends `<div class="msg-translation-annex">`
- Second translate click: removes the annex element

**`addUserCorrectionIcon(wrapper, correction)`**

Called after the AI responds. If `correction.original !== correction.corrected`, appends a pencil icon button below the user's bubble. Clicking it toggles an amber-styled correction card showing the corrected sentence and bullet notes.

**`typewriterReveal(element, text, onDone, scrollContainer)`**

Splits text by space, sets each word with a 28ms delay. Scrolls `scrollContainer` to bottom on each step. Calls `onDone` when complete.
...
11. if speech: renderSpeechFeedback() + custom audio player loads src
12. setBusy(false)
```
...
Entry point. Loaded last. Contains:

- `bindModalEvents()` — wires save/discard/cancel buttons in the unsaved-changes modal
- `bindEvents()` — all `addEventListener` calls for the entire app, including history buttons
- `init()` — loads config → options → hydrates selectors → binds events → inits audio player → refreshes memory
- `initCustomAudioPlayer()` — wires the custom `#capPlayer` UI to the native `<audio>` element events (loadedmetadata, timeupdate, play, pause, ended, emptied)
- `openHistoryModal()` / `closeHistoryModal()` — calls `window.speakAI.listConversations()` and renders results in `#historyModalList`
...
[Renderer] removeTypingIndicator()
           addChatMessage("assistant", ..., animate=true)  ← typewriter reveal
           addUserCorrectionIcon(userMsgWrapper, correction)
           renderCorrectionBox(...)
           renderSpeechFeedback(...)     [speech only]
           custom audio player loads src + plays  [speech only, if autoPlay]
```
...
The conversation history modal (`#historyModal`) calls `window.speakAI.listConversations()` which reads the same `talk_N.txt` files and returns a preview (first 300 chars) of each.
...
When `correction.original !== correction.corrected`, a pencil icon appears below the user's message bubble. Clicking it opens an inline amber card with the corrected sentence and explanation notes.
...
| `--surface-soft` | Subtle surface (AI message bubbles, audio player) |
| `--surface-strong` | Stronger surface (hover states, progress track) |
| `--border` | Default border color |
| `--text-main` | Primary text |
| `--text-soft` | Secondary/placeholder text |
| `--accent` | Brand color (buttons, scrollbars, chips, active states) |
| `--accent-strong` | Darker accent for hover states |
| `--ok` | Success status color (also API key confirmed badge) |
| `--warn` | Error/warning color (also correction card accent) |
| `--radius-sm` / `--radius-md` / `--radius-lg` / `--radius-xl` | Border radius scale |
| `--shadow-sm` / `--shadow-md` | Box shadow values |

Scrollbars use `scrollbar-color: var(--accent) transparent` (Firefox) and `::-webkit-scrollbar-thumb { background: var(--accent) }` (Chromium/Electron). Applied consistently to: sidebar, chat messages, coach box, options panel, language chip list, history modal list.
